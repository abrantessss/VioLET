...
namespace autopilot {
  ...
  void LOS3Controller::initialize() {
    ...
    node_->declare_parameter<double>("controllers.los3controller.Tmax", 15.0);
    ...
    Tmax_ = node_->get_parameter("controllers.los3controller.Tmax").as_double();
    ...
    if (Tmax_ <= 0.0) {
      RCLCPP_WARN_STREAM(
        node_->get_logger(),
        "LOS3Controller Tmax must be positive, using default Tmax = 15.0");
      Tmax_ = 15.0;
    }
    ...
    // Log Gains 
    RCLCPP_INFO_STREAM(node_->get_logger(), "LOS3Controller vehicle mass: m = " << m_);
    RCLCPP_INFO_STREAM(node_->get_logger(), "LOS3Controller max thrust: Tmax = " << Tmax_);
